        ...
        <div class="form-group">
            ...
            <div class="form-group">
              <label class="control-label col-sm-4" for="txt">Web:</label>
                <div class="col-sm-6"> 
                  <input type="url" class="form-control" id="wesite" name="web" placeholder="www.example.com" required>
                </div>
           </div>
        <!-- club social -->
...
<script>
  ...
  url: function(phrase) {
    return "{{ url('clubaddress/address') }}/"+phrase;
  },
  ...
  list: {
    onSelectItemEvent: function() {
      ...
      var postcode = $("#address").getSelectedItemData().PostCode;
       ...
       $("#club-city").val(city).trigger("change");
       $("#club-town").val(town).trigger("change");
       $("#post-code").val(postcode).trigger("change");
       $("#club-country").val(country).trigger("change");
    }
  }
  };
  ...
  url: function(phrase) {
    return "{{ url('clubaddress/contact') }}/"+phrase;
  },
  getValue: "Phone",
  list: {
    onSelectItemEvent: function() {
      var site = $("#club-mobile").getSelectedItemData().Website;
      var email = $("#club-mobile").getSelectedItemData().Email;
     
       $("#wesite").val(site).trigger("change");
       $("#email").val(email).trigger("change");
    }
  }
  };
  $("#club-mobile").easyAutocomplete(options); 
});
</script>
@endsection